...
def activity_overview(tag,title,desc,build,services,kicker):
    ...
    # Give the output and tool descriptions independent rows. Several labs have
    # deliberately explicit deliverables, so fixed 18 pt text in a 0.6-inch box
    # can wrap into the Tools row even though the source content is valid.
    rect(s,Inches(0.85),Inches(4.25),Inches(11.7),Inches(2.35),_panel())
    txt(s,Inches(1.1),Inches(4.43),Inches(11),Inches(0.35),[[("You'll build",14,_acc(BLUE),True)]])
    bL=len(str(build)); bsize=16 if bL<=130 else (14 if bL<=190 else 12)
    txt(s,Inches(1.1),Inches(4.78),Inches(11),Inches(0.9),[[(build,bsize,_ink(),True)]])
    sL=len(str(services)); ssize=12 if sL<=150 else 10
    txt(s,Inches(1.1),Inches(5.72),Inches(11.2),Inches(0.75),[[("Tools:  ",ssize,_grey(),True),(services,ssize,_grey(),False)]]); footer(s); return s
...
for t in C.TOPICS:
    ...
    for a in acts:
        ...
        route.append(a.get("deck_test", "Complete the lab's Test It checklist."))
