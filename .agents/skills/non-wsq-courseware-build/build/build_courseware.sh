...
# Use an isolated LibreOffice profile. Multiple course builds can run on this
# workstation at once; sharing the default profile makes a headless conversion
# attach to another process and fail without a useful error.
LO_PROFILE="$(mktemp -d)"
trap 'test -n "${LO_PROFILE:-}" && rm -rf -- "$LO_PROFILE"' EXIT
if command -v cygpath >/dev/null 2>&1; then
  LO_PROFILE_URL="file:///$(cygpath -m "$LO_PROFILE")"
else
